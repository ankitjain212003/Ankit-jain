...
export class EmployeesComponent implements OnInit {
    ...
    dialogRef.afterClosed().subscribe(employ => {
      if(employ) {
        if (employ.id ===0){
          ...
          this.employeesService.createEmploy(employ).subscribe(employees => this.dataSource.data = employees)
        } else {
  ...
  remove(employee: Employ){
    ...
    dialogRef.afterClosed().subscribe(employ => {
      if(employ) {
        this.employeesService.deleteEmploy(employee.id).subscribe(employee =>this.dataSource.data = employee)
      }
    })
  }
